...
class connector():
    ...
    _options = {
        ...
        'disabled': [],
        'debug': False,
        'noHash':False ###Disable / Enable Hashing for Speed Improvement Use at Your Own RISK!
    }
    _commands = {
        'open':    '__open',
        'reload': '__reload',
    ...
    def __open(self):
        ...
        # try to open file
        self._response['error'] =None ###INFO: Refreshed self._response['error'] because it got stucked when one error happens 
        self.httpStatusCode = None

#        is_dir=os.path.isdir(curFile)
###TODO: Do Not Give Error when curFile is a Dir , do Dir Instead 
###-- DONE
        if 'current' in self._request:  
            curDir = self.__findDir(self._request['current'], None)
            curFile = self.__findDir(self._request['target'], None)
#            if not curDir or not curFile or os.path.isdir(curFile):
###FIXED: Response Error when curfile is not a file but a dir 
###FIXED: dont return dir list after __upload/__rename/__mkfile
            if not curDir or not curFile :# or os.path.isdir(curFile):
                self.httpStatusCode = 404
                self.httpHeader['Content-type'] = 'text/html'
                self.httpResponse = 'File not found'
                return
            elif curDir or curFile or os.path.isdir(curFile):
...
#            self._response['select'] = [self.__hash(archivePath)]
    ...
    def __hash(self, path):
        """Hash of the path"""
        if self._options['noHash'] == True: #Disable / Enable Hashing for Speed Improvement Use at Your Own RISK!
            return path
        else :
            m = hashlib.md5()
            m.update(path)
...
###FIXED:Checking all archivers take so much time and bzip2 --version freeze elFinder for me DISABLED
